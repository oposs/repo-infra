...
**Type consistency.** The `{read, write}` object is `io` inside `bump.js` (Task 5), where it is an ordinary parameter, and `fileIO` inside every `script:` block (Tasks 9, 10), where `io` is a name `actions/github-script` injects — see the warning at Task 9. `spec` is `{path, pattern, replacement, verify}` in `.github/repo-infra.json` (Task 1), `bump.js` (Task 5) and both workflows. `checkState` takes `(github, params, opts)` and returns `{total, pending, failed, ok}`; `waitForChecks` adds `timedOut` and forwards `opts`, whose `ignoreCheckRunIds` is how the Task 9 guard avoids waiting for its own job (Task 6), both consumed in Task 9. `commitFiles` takes `{owner, repo, branch, baseSha, message, files}` and returns a SHA (Task 7), called that way in Task 9. `next()` returns a bare version and every heading, tag and API call adds the `v` itself.
