...
package, that is the signal to containerize.**
...
ecosystem is exempt — the threshold decides.
...
1. **`assets/build/*.mk`** — the container test fragment, generalised from
   ...
   A project that crosses the D16 threshold installs it and writes its own
   `Dockerfile`.
...
1. **Does `check` need to detect the threshold?** D16 is a judgement — "does
   this project need an extra system package" is not a file signal. `check` can
   report that a repository has a `Dockerfile` and the fragment, or has neither,
   but it cannot say whether it *should*. Leaving that to the conversion reader
   is consistent with the detection model; stating it here so it is a decision
   rather than an omission.
2. **SmokePing's release tarball.** Its current release attaches
   `smokeping-X.Y.Z.tar.gz`. `release-publish.yml` creates a draft with no
   artifact, because publish add-ons are spec 2 and unbuilt. Converting
   SmokePing before then is a regression its users would notice. Spec 2's add-on
   list also has no entry for an autotools `make dist` tarball.
